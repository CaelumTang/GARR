...
class SwiftMixin:
    ...
    def __init__(self,
        ...
        custom_hidden_loss = getattr(args, 'loss_type', None)
        if custom_hidden_loss == 'garr_loss':
            base_forward = self.model.forward

            def _wrapped_forward(this, *fargs, **fkwargs):
                fkwargs.setdefault('return_dict', True)
                fkwargs.setdefault('output_hidden_states', True)
                out = base_forward(*fargs, **fkwargs)
                ...
                if hasattr(out, 'hidden_states') and out.hidden_states is not None:
                    ...
                    raise RuntimeError(
                        f'{custom_hidden_loss} requires outputs.hidden_states (set output_hidden_states=True).')

                last_hs = hs[-1]
                if isinstance(out, dict):
                    out['last_hidden_state'] = last_hs
                else:
                    out.last_hidden_state = last_hs

                if 'input_ids' in fkwargs:
                    if isinstance(out, dict):
                        out['input_ids'] = fkwargs['input_ids']
                    else:
                        out.input_ids = fkwargs['input_ids']
                return out

            self.model.forward = MethodType(_wrapped_forward, self.model)
            logger.info_once(f'Patched model.forward to expose last_hidden_state for {custom_hidden_loss}.')

            learnable_env = os.environ.get('GARR_CON_LEARNABLE', '1')
            learnable = bool(isinstance(learnable_env, str) and learnable_env.lower() in ('1', 'true', 'yes'))
            tau_init = float(os.environ.get('GARR_CON_T_INIT', os.environ.get('GARR_CON_T', '0.15')))
            inv_t_max = float(os.environ.get('GARR_CON_INV_T_MAX', '100.0'))

            model_for_temp = self.model.module if hasattr(self.model, 'module') else self.model
            dev = next(model_for_temp.parameters()).device

            if not hasattr(model_for_temp, 'garr_con_inv_t_max'):
                model_for_temp.register_buffer(
                    'garr_con_inv_t_max', torch.tensor(inv_t_max, dtype=torch.float32, device=dev)
                )

            if learnable and not hasattr(model_for_temp, 'garr_con_logit_scale'):
                init_logit = float(math.log(1.0 / tau_init))
                model_for_temp.garr_con_logit_scale = nn.Parameter(
                    torch.tensor(init_logit, dtype=torch.float32, device=dev)
                )
                logger.info_once(
                    f'Initialized GARR learnable temperature: tau_init={tau_init}, '
                    f'inv_t_max={inv_t_max}, logit_scale={init_logit:.6f}')
