...
def load_file(path: Union[str, bytes, _T]) -> Union[BytesIO, _T]:
    ...
    if isinstance(path, str):
        ...
        # - h5://<file>::<key>
        # - h5:/<file>::<key>
        # - /some/prefix/h5:/<file>::<key> (strip the prefix before the last 'h5:')
        if ('::' in path) and ('h5:' in path):
            left, key = path.split('::', 1)
            # Keep the part after the last 'h5:' to avoid accidental prefix concatenation.
            left = left[left.rfind('h5:') + 3:]
            if left.startswith('//'):
                left = left[2:]
            file_part = left
            file_part = os.path.abspath(os.path.expanduser(file_part))
            try:
                import h5py  # lazy import
                import numpy as np  # for robust bytes conversion
            except Exception as e:  # pragma: no cover
                raise RuntimeError('h5py is required to load h5:// urls') from e
            with h5py.File(file_part, 'r') as h5f:
                data = h5f[key][()]
            if isinstance(data, (bytes, bytearray)):
                res = BytesIO(data)
            else:
                try:
                    res = BytesIO(bytes(data))
                except Exception:
                    res = BytesIO(np.array(data).tobytes())
